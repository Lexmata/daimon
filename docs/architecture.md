...
│   ├── model/              # Model trait re-exports + OpenAI, Anthropic, Ollama
...
| **daimon-provider-*** | LLM provider crates. Each implements `Model` and `EmbeddingModel` from daimon-core. Depend only on daimon-core + HTTP/SDK. |
...
```
    ...
    │              ▼                               ▼                       │
    │   ┌─────────────────────┐         ┌─────────────────────────────────┐│
    │   │ has_tool_calls?     │   NO   │  Add assistant message to memory ││
...
- **No tool calls** — Model returns text; agent adds message to memory, runs output guardrails, returns `AgentResponse`.
...
```rust
/// Tool the agent can invoke. JSON Schema for parameters.
...
| **openai** | reqwest | OpenAI API provider (GPT-4, etc.) |
| **anthropic** | reqwest | Anthropic Claude API |
...
| **ollama** | reqwest | Ollama local models |
...
| **grpc** | tonic, prost | gRPC transport for distributed execution |
...
├── reqwest (openai | anthropic | ollama | mcp)
├── rusqlite (sqlite)
├── redis (redis)
├── async-nats (nats)
├── lapin (amqp)
...
pub trait ChainStep: Send + Sync {
    fn execute(&self, ctx: &mut ChainContext) -> impl Future<Output = Result<()>> + Send;
}
```
...
pub trait GraphNode: Send + Sync {
    fn execute(&self, ctx: &mut GraphContext) -> impl Future<Output = Result<NodeOutcome>> + Send;
}
```
...
pub trait WorkflowNode: Send + Sync {
    fn execute(&self, ctx: &mut WorkflowContext) -> impl Future<Output = Result<()>> + Send;
}
```
...
```rust
let broker = RedisBroker::new(redis_url)?;
loop {
...
Wraps an agent with `Checkpoint`. When `prompt` is called with a `run_id`, it loads the checkpoint (if any), restores the message history, and continues from the last state.
...
- **EvalResult** — Pass/fail, latency, cost, score.
...
| `ChatResponse` | message, stop_reason (EndTurn/ToolUse/MaxTokens), usage |
...
Wraps another memory backend. Truncates history to fit within a token budget (estimated by character count). Ensures the context never exceeds model limits.
...
One agent delegates to specialized sub-agents. The supervisor receives a task, routes it to a sub-agent by name, and returns the result. Sub-agents are `HashMap<String, Arc<Agent>>`.

### HandoffNetwork

Agents transfer control to each other mid-conversation. A handoff action specifies the target agent and context. Use for multi-agent conversations where control passes between participants.

### StructuredOutput

Extracts typed data from LLM responses. Uses a schema (e.g., JSON Schema) to parse the model's text into a struct. Fails if the output does not match.
...
- **Single DaimonError** enum; providers map to `Model` or `Other`.
